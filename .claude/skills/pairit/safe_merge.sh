...
# The head is read ONCE; every later check and the merge itself are tied to it, so
# a commit pushed meanwhile can't be merged unchecked or unreviewed (#186)
...
case $? in
  ...
  *) echo "NOT MERGING #$PR: git merge-tree failed (git 2.38+ needed)"; exit 1 ;;
esac
...
verdict=$(printf '%s' "$pr" | jq -r '[.comments[]?.body | select(startswith("reviewed-at-sha:"))]
  | last // "" | split("\n") | .[0:2] | map(sub("\r$"; "")) | join("\n")')
