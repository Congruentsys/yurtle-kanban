...
def hdd_backfill(dry_run):
    ...
    if backfilled:
        ...
        for r in backfilled:
            ...
            table.add_row(
                escape(str(r["id"])), escape(str(r["type"])), str(r["triples_added"]), action_text
            )
        console.print(table)
...
def _render_critical_path(
    ...
    for readiness_key, (label, color, desc) in group_labels.items():
        ...
        for exp in items:
            ...
            console.print(
                f"  [{color}]●[/{color}] [bold]{escape(str(eid))}[/bold]"
                f" — {escape(str(exp['title']))}"
            )
...
def experiment_status(expr_id: str, as_json: bool):
    ...
    for run in runs:
        ...
        table.add_row(
            escape(str(run["timestamp"])[:19]),
            escape(str(run["being"])),
            f"[{status_style}]{escape(str(run['status']))}[/{status_style}]",
            escape(str(outcome or "")),
        )
