...
def hdd_backfill(dry_run):
    ...
    if backfilled:
        ...
        for r in backfilled:
            ...
            table.add_row(
                safe(str(r["id"])), safe(str(r["type"])), str(r["triples_added"]), action_text
            )
        console.print(table)
...
def _render_dev_blockers(blockers: list[dict]) -> None:
    ...
    for i, b in enumerate(blockers, 1):
        ...
        console.print(
            f"  {i}. [bold]{safe(str(b['expedition_id']))}[/bold] — {safe(str(b['title']))}"
        )
        ...
        console.print(
            ...
            + safe(", ".join(str(x) for x in b["unblocks_experiments"]))
        )
        console.print()
...
def _render_critical_path(
    ...
    for readiness_key, (label, color, desc) in group_labels.items():
        ...
        for exp in items:
            ...
            console.print(
                f"  [{color}]●[/{color}] [bold]{safe(str(eid))}[/bold]"
                f" — {safe(str(exp['title']))}"
            )
...
def experiment_status(expr_id: str, as_json: bool):
    ...
    for run in runs:
        ...
        table.add_row(
            safe(str(run["timestamp"])[:19]),
            safe(str(run["being"])),
            f"[{status_style}]{safe(str(run['status']))}[/{status_style}]",
            safe(str(outcome or "")),
        )
